...
export async function POST(request: Request) {
  try {
    ...
    // Simple secret to prevent unauthorized initialization
    ...
    `, [username || 'lds', hashedPassword]);

    // Get user ID
    const userResult = await pool.query('SELECT id FROM users WHERE username = $1', [username || 'lds']);
